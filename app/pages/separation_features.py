...
def get_audio_from_youtube(video_url):
    ...
    downloaded_file = video.download(output_path="audio/youtube", filename="audio")
    audio_file_path = os.path.splitext(downloaded_file)[0] + ".mp3"
    ...
    with selected_section[0]:
        ...
        if source_url:
            if st.button(
                ...
                #     source_url, "audio/youtube"  # change the output directory
                # )
                audio_file_path = get_audio_from_youtube(source_url)
                st.write("Orginal Audio")
                st.audio(audio_file_path, format="audio/wav")
